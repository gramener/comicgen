  ...
  function comic(template, _replacements) {
      ...
      _.each(_replacements, (replace) => {
        if (replace.value && replace.selector) {
          ...
          replace.attr.split(/[, ]+/g).forEach((attr) => {
            if ($el.is(`[${attr}]`)) $el.attr(attr, replace.value);
          });
        }
      });
      return $.html().trim();
    ...
    // Else, if we get a dict of options...
      ...
      try {
        ...
        throw new ComicError(`Unknown character ${template.name}`, {
          name: template.name,
        });
      }
      // If it’s a directory, read the `index.svg`. Else read the file intself
      if (stat.isDirectory()) svg_path = path.join(svg_path, "index.svg");
      ...
      const viewBox =
        config.defaults.width && config.defaults.height
          ? `viewBox="0 0 ${config.defaults.width} ${config.defaults.height}"`
          : "";
      ...
      let box = "";
      if (attrs.box) {
        const gen = roughjs.generator({
          options: {
            seed: 1,
            bowing: 2,
            roughness: Math.max(2, Math.min(attrs.box / 2, 5)),
          },
        });
