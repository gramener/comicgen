...
function wrap(sentence, maxWidth, font) {
  ...
  for (let start=0, word=0, line=[]; word < words.length; ) {
...
function speechpath({x = 0, y = 0, width, height, pointerx, pointery, fill = '#fff', stroke = '#000'}) {
  const paths = [
    `M ${x},${y}`,
    `L ${x + width},${y}`,
    `L ${x + width},${y + height}`,
...
function textwrap(options) {
  let font = fonts[options['font-family']]
  let lineHeight = (+options['font-size'] || 16) * (+options['line-height'] || 1.5)
  let maxWidth = options.width * font.unitsPerEm / options['font-size']
  let align = options.align || 'middle'
  // Get the attributes of <text...> and <path...> into attrs.text and attrs.path
  let attrs = {
    text: [`text-anchor="${align}"`, `dx="${dx[align] * options.width}"`],
    path: [],
  }
  ...
  // Render the path
  return speechpath(options) + text
}
...
// [w, h]
// totalwidth x linespacing = w x h
// linespacing = w x h / totalwidth
